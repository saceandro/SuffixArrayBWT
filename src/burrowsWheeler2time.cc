...
int* inducedSortLms(int* s, int len, int types, int first)
{
  int i,j;
  ...
  if (rank+1 == lmslen)
    ...
    {
      int* ret = inducedSortLms(s2, lmslen, rank+1, 0);
      for (i=0; i<lmslen; i++)
...
int main(int argc, char** argv)
{
  ...
  int* sa = inducedSortLms(s, len, 6, 1);
  ...
  int querylen;
  for (querylen=10; querylen<=100000; querylen*=10)
    {
      ...
      for (i=0; i<sample; i++)
        {
          randArray(encoded_query, querylen);
          
          gettimeofday(&tv1, NULL);
          int start = lb(encoded_query, querylen, occ, len, count);
          int end = ub(encoded_query, querylen, occ, len, count);
